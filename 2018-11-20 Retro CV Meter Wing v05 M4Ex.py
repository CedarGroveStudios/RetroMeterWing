# 2018-11-20 Retro CV Meter Wing v05 M4Ex.py
...
# ctl_idx (Control Index) converts an analog value to index integer
#  Input is masked to 8 bits to reduce noise then hysteresis offset
#  is applied. (ItsyBitsy M4 DAC and ADC are 12-bit; CircuitPython
#  converts to 16-bits.) The helper returns new index value, input
#  hysteresis offset based on max, and index change flag.
def ctl_idx(ctl, max, offset, old_idx):
    ...
    if idx != old_idx:  # if index changed, adjust hysteresis offset and set flag
        # offset is 10% of the control slice (65536/max)
        offset = int(0.1 * sign(idx - old_idx) * (65535 / max))
        flag = True  # index value has changed
    else:
        flag = False  # index value is the same
    return idx, offset, flag
...
print("2018-11-20 Retro CV Meter Wing v05 M4Ex.py")
...
print("Movement rate: %0.3f" % move_rate, "seconds/volt; Output hold: %0.3f" % output_hold, "seconds")
print("Minimum movement increment: %0.3f" % move_inc, "volts")
print("Initial meter scale: %1.0f" % meter_low, "to %1.0f" % meter_high, "volts")
...
while True:
    if time.monotonic() > t0 + 0.500:  #read the scale potentiometer every 500msec
        ...
        (scale_idx, scale_offset, scale_flag) = ctl_idx(scale_pin.value, scale_max, scale_offset, scale_idx_old) # determine scale index
        if scale_flag:  # if the index changed, blink the amber led
            amber_led.duty_cycle = 0
            click()
            amber_led.duty_cycle = 65535
            meter_low, meter_high = scales[scale_idx]  # set the meter scale limits
            print("Scale: %1.0f" % meter_low, "to %1.0f" % meter_high, "volts")
